...
class Ghost:
    ...
    # r, l, u, d
    # blinky will turn wheneve collide wiht walls otheriwzse continue straight.
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
            elif not self.turns[0]:
    ...
    def move_inky(self):
        # r, l, u, d
        # Inky turns up or down at any point to pursue but left or right only on collision
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
            elif not self.turns[0]:
    ...
    def move_pinky(self):
        # r, l, u, d
        # Pinky is going to turn left or right when best but only up or down on collision
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
            elif not self.turns[0]:
...
def check_collide(scor, power, power_count, eaten_ghosts):
    num1 = (HEIGHT - 50) // 32
    num2 = WIDTH//30
    if 0 < player_x < 870:
...
while run:
    ...
    score, powerup, power_counter, eaten_ghost = check_collide(score, powerup, power_counter, eaten_ghost)
